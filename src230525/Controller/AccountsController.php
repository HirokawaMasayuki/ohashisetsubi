	...
	*/
         ...
         $tuika = $data['num'];
				...
				if(isset($data["zenkai0"])){
					...
					return $this->redirect(['action' => 'uriageformzenkaikensaku',
					's' => ['get' => 1, 'id' => $id]]);
						...
						return $this->redirect(['action' => 'uriageformzenkai',
						's' => ['get' => 1, 'id' => $id]]);
					 ...
					 return $this->redirect(['action' => 'uriageformzenkaisyousai',
					 's' => ['get' => 1, 'id' => $id]]);
...
//		 $Uriagemasters = $this->Uriagemasters->find()->where(['customerId' => $data["id"], 'delete_flag' => 0])->order(["created_at"=>"desc"])->limit(10)->toArray();
	...
	//	 $arrUriagesyousais = $this->paginate($arrUriagesyousais);
...
*/
			 if(isset($data["tugihe"])){
				 ...
				 return $this->redirect(['action' => 'uriageformsyousai',
			 ...
			 $tourokuArr = array('denpyou_num' => $denpyou_num,'bunrui' => $data["bunrui"],
			 'customerId' => $data["id"],'customer' => $data["name"],'furigana' => $data["furigana"]
			 ,'yuubin' => $data["yuubin"],'address' => $data["address"],'tax_include_flag' => $tax_include_flag
			 ,'keisyou' => $data["keisyou"],'uriagebi' => $data["datetouroku"],'delete_flag' => 0,
	...
	//			$sheet->setCellValue('A5', $namearr[0]);
			 ...
			 if($syou == 0){//最後のシートの時
				 ...
				 if($data["tax_include_flag"] == 1){//内税の場合
					 ...
					 $sheet->setCellValue("H22", $total_price);
					 $sheet->setCellValue("H44", $total_price);
					 $sheet->setCellValue("H45", "");
					 $sheet->setCellValue("H46", $total_price);

				 }else{

					 $sheet->setCellValue("H20", $total_price/1.1);
					 $sheet->setCellValue("H21", $total_price - $total_price/1.1);
					 $sheet->setCellValue("H22", $total_price);
					 $sheet->setCellValue("H44", $total_price/1.1);
					 $sheet->setCellValue("H45", $total_price - $total_price/1.1);
					 $sheet->setCellValue("H46", $total_price);
		...
		/*
					 if($data["tax_include_flag"] == 1){//内税の場合
						 ...
						 $sheet->setCellValue("H20", $total_price_taxinc);
						 $sheet->setCellValue("H21", "");
						 $sheet->setCellValue("H22", $total_price_taxinc);
						 $sheet->setCellValue("H44", $total_price_taxinc);
						 $sheet->setCellValue("H45", "");
						 $sheet->setCellValue("H46", $total_price_taxinc);
	...
	*/
     ...
     public function motouriagekensakuview0()//売上照会
     {
			 $uriages = $this->Uriages->newEntity();
       $this->set('uriages',$uriages);
       $data = $this->request->getData();
...
*/
  ...
  //     $date_fin = date('Y-m-d', strtotime('+1 day', $date_fin));
					 ...
					 if(empty($data['denpyou_num'])){//denpyou_numの入力がないとき
						 ...
						 if(empty($data['furigana'])){//furiganaの入力がないとき
							 ...
							 if(empty($data['customer'])){//customerの入力がないとき
								 ...
								 if(empty($data['proname'])){//pronameの入力がないとき
									 ...
									 $Uriages = $this->Uriagemasters->find()->limit(300)
									 ->where(['uriagebi >=' => $date_sta, 'uriagebi <=' => $date_fin, 'delete_flag' => 0, 'bunrui like' => '%'.$bunrui.'%'])->order(["uriagebi"=>"DESC"])->toArray();
									 $this->set('Uriages',$Uriages);
								...
								if(empty($data['proname'])){//pronameの入力がないとき
									...
									$Uriages = $this->Uriagemasters->find()->limit(300)
									->where(['uriagebi >=' => $date_sta, 'uriagebi <=' => $date_fin,  'customer like' => '%'.$customer.'%',  'delete_flag' => 0, 'bunrui like' => '%'.$bunrui.'%'])->order(["uriagebi"=>"DESC"])->toArray();
									$this->set('Uriages',$Uriages);
						 ...
						 if(empty($data['customer'])){//customerの入力がないとき
							 ...
							 if(empty($data['proname'])){//pronameの入力がないとき
								 ...
								 $Uriages = $this->Uriagemasters->find()->limit(300)
								 ->where(['uriagebi >=' => $date_sta, 'uriagebi <=' => $date_fin,  'furigana like' => '%'.$furigana.'%',  'delete_flag' => 0, 'bunrui like' => '%'.$bunrui.'%'])->order(["uriagebi"=>"DESC"])->toArray();
								 $this->set('Uriages',$Uriages);
							...
							if(empty($data['proname'])){//pronameの入力がないとき
								...
								$Uriages = $this->Uriagemasters->find()->limit(300)
								->where(['uriagebi >=' => $date_sta, 'uriagebi <=' => $date_fin,  'furigana like' => '%'.$furigana.'%',  'customer like' => '%'.$customer.'%',  'delete_flag' => 0, 'bunrui like' => '%'.$bunrui.'%'])->order(["uriagebi"=>"DESC"])->toArray();
								$this->set('Uriages',$Uriages);
						 ...
						 if(empty($data['furigana'])){//furiganaの入力がないとき
							 ...
							 if(empty($data['customer'])){//customerの入力がないとき
								 ...
								 if(empty($data['proname'])){//pronameの入力がないとき
									 ...
									 $Uriages = $this->Uriagemasters->find()->limit(300)
									 ->where(['uriagebi >=' => $date_sta, 'uriagebi <=' => $date_fin, 'denpyou_num' => $denpyou_num,  'delete_flag' => 0, 'bunrui like' => '%'.$bunrui.'%'])->order(["uriagebi"=>"DESC"])->toArray();
									 $this->set('Uriages',$Uriages);
								...
								if(empty($data['proname'])){//pronameの入力がないとき
									...
									$Uriages = $this->Uriagemasters->find()->limit(300)
									->where(['uriagebi >=' => $date_sta, 'uriagebi <=' => $date_fin ,  'customer like' => '%'.$customer.'%', 'denpyou_num' => $denpyou_num,  'delete_flag' => 0, 'bunrui like' => '%'.$bunrui.'%'])->order(["uriagebi"=>"DESC"])->toArray();
									$this->set('Uriages',$Uriages);
						 ...
						 if(empty($data['customer'])){//customerの入力がないとき
							 ...
							 if(empty($data['proname'])){//pronameの入力がないとき
								 ...
								 $Uriages = $this->Uriagemasters->find()->limit(300)
								 ->where(['uriagebi >=' => $date_sta, 'uriagebi <=' => $date_fin,  'furigana like' => '%'.$furigana.'%', 'denpyou_num' => $denpyou_num,  'delete_flag' => 0, 'bunrui like' => '%'.$bunrui.'%'])->order(["uriagebi"=>"DESC"])->toArray();
								 $this->set('Uriages',$Uriages);
							...
							if(empty($data['proname'])){//pronameの入力がないとき
								...
								$Uriages = $this->Uriagemasters->find()->limit(300)
								->where(['uriagebi >=' => $date_sta, 'uriagebi <=' => $date_fin,  'furigana like' => '%'.$furigana.'%', 'denpyou_num' => $denpyou_num,  'customer like' => '%'.$customer.'%',  'delete_flag' => 0, 'bunrui like' => '%'.$bunrui.'%'])->order(["uriagebi"=>"DESC"])->toArray();
								$this->set('Uriages',$Uriages);
		...
		//	 $Uriagetotalhyouji = $Uriagetotalhyouji * 1.1;
			 $Uriagetotalhyouji = $Uriagetotalhyouji;
				 ...
				 public function uriagekensakuedit()
		     {
		       ...
		       $this->set('uriages',$uriages);
					 ...
					 if(isset($dataidatesaki[0])){
						 ...
						 return $this->redirect(['action' => 'uriagekensakueditcustomerform',
		         ...
		         '管' => '管'
  		         ...
  		         1 => '内税'
					 ...
					 if(isset($data["zenkai3"])){//行追加
						 ...
						 return $this->redirect(['action' => 'uriagekensakuedittuika',
	 					's' => ['get' => 1]]);
...
/*
						 echo "<pre>";
						 print_r($_SESSION['editpass']);
						 echo "</pre>";
*/
 						 return $this->redirect(['action' => 'uriagekensakueditzenkaikensaku',
 	 					's' => ['get' => 1]]);
						 ...
						 $_SESSION['editpass'] = $data;
						 return $this->redirect(['action' => 'uriagekensakueditconfirm',
	 					's' => ['get' => 1]]);
	...
	//	       $this->set('tax_include_flag',$tax_include_flag);
					 ...
					 for($i=0; $i<=$data['num']; $i++){
						...
						$Uriagesyousais[] = ['pro' =>  $data['pro_'.$i], 'amount' =>  $data['amount_'.$i], 'tani' =>  $data['tani_'.$i],
							'tanka' =>  $data['tanka_'.$i], 'bik' =>  $data['bik_'.$i], 'delete_flag' =>  $data['delete_flag'.$i],
					...
					if(isset($getData["editzenkaikobetu"])){//選択したとき
 					 ...
 					 for($i=0; $i<count($arreditzenkaikobetu); $i++){
						 ...
						 $Uriagesyousais[] = ['pro' =>  $Uriagesyousaiskensaku[0]["pro"], 'amount' => $Uriagesyousaiskensaku[0]["amount"],
 						 'tani' =>  $Uriagesyousaiskensaku[0]["tani"],
 							'tanka' => $Uriagesyousaiskensaku[0]["tanka"], 'bik' => $Uriagesyousaiskensaku[0]["bik"], 'delete_flag' => 0,
 							'id'  => "new_data"];
 							$this->set('Uriagesyousais',$Uriagesyousais);
					 ...
					 $_SESSION['editpass'] = $data;
						...
						$Uriagesyousais[] = ['pro' =>  "", 'amount' => "", 'tani' =>  "",
							'tanka' => "", 'bik' => "", 'delete_flag' => "",
							'id'  => "new_data"];
							$this->set('Uriagesyousais',$Uriagesyousais);
									 ...
									 $arrnew_data = [
										...
										if(strlen($data['tanka_'.$i]) > 0 && strlen($data['amount_'.$i]) > 0){
										 ...
										 $this->Uriagesyousais->updateAll(
											...
											'tanka' =>  $data['tanka_'.$i], 'price' =>  ${"price".$i}, 'bik' =>  $data['bik_'.$i], 'delete_flag' =>  $data['delete_flag'.$i]],
										 	['id'  => $data['uriagesyousaiId'.$i]]
										);
...
/*//220211コメントアウト
						 ...
						 $count = count($Nyuukins);
						 for ($k=0; $k<$count; $k++){
							 $nyuukinntotal = $nyuukinntotal + $Nyuukins[$k]->nyuukinngaku;
						 }
						 ...
						 for ($k=0; $k<$count; $k++){
							 $tyouseitotal = $tyouseitotal + $Nyuukinstyousei[$k]->nyuukinngaku;
						 }
						 ...
						 for ($k=0; $k<$count; $k++){
							 $sousaitotal = $sousaitotal + $Nyuukinssousai[$k]->nyuukinngaku;
						 }
*/
					 }else{
						 $Zenkai = 0;
...
*/
					 if(checkdate($data["date"]["month"], $data["date"]["day"], $data["date"]["year"])) {
						 ...
						 return $this->redirect(['action' => 'seikyuuform',
						 's' => ['name1' => $data["id"], 'mess' => $mess]]);
 						 ...
 						 $sheet->setCellValue('A4', "　　");
 							...
 							if(isset($namearr[1])){
								...
								$sheet->setCellValue('F7', $keisyou);
 								...
 								$sheet->setCellValue('B29', "　".$namearr[0]);
 								$sheet->setCellValue('B30', "　　 ".$namearr[1]);
								$sheet->setCellValue('F29', $keisyou);
								...
								if($str_mblen_name > 17){
								 ...
								 $sheet -> getStyle("B7") -> getAlignment() -> setHorizontal(Align::HORIZONTAL_CENTER);
	 							 ...
	 							 $sheet->unmergeCells('B29:E29');
	 							 $sheet->unmergeCells('B30:E30');
	 							 $sheet->mergeCells('B29:E30');
	 							 $sheet->setCellValue('B29', "　".$namearr[0]);
								 $sheet -> getStyle("B29") -> getAlignment() -> setShrinkToFit(true); //縮小して表示
							 ...
							 for ($k=0; $k<1; $k++){
								 ...
								 for($i=0; $i<count($Uriagesyousais); $i++){
									 ...
									 if(!empty($Uriagesyousais[$i]->pro)){
										 ...
										 ${"arrPros".$k}[] = array('pro' => $Uriagesyousais[$i]->pro, 'amount' => $Uriagesyousais[$i]->amount,
										 'tani' => $Uriagesyousais[$i]->tani, 'tanka' => $Uriagesyousais[$i]->tanka,
										 'price' => $Uriagesyousais[$i]->price, 'bik' => $Uriagesyousais[$i]->bik);
						...
						if($seikyuusakicount > 0){//請求先をまとめている場合
						 ...
						 $sheet->setCellValue('Q6', $keisyou);
		 					 ...
		 					 if($data["datehyouji_flag"] == 1){
								 ...
								 $sheet->setCellValue('J3', $dateexcl."");
								 $sheet->setCellValue('U3', $dateexcl."");
		 					 }

							 $sheet->setCellValue('E14', $data["totalkingakutaxinc"]);
							 $sheet->setCellValue('P14', $data["totalkingakutaxinc"]);
	 		 				 ...
	 		 				 for($i=0; $i<20; $i++){
								 ...
								 if($arrTaxinc[$s] > 0){//内税の場合
									 ...
									 if($amari == 0 && $syou == 0){
										 ...
										 $sheet->setCellValue('H39', "合計");
										 $sheet->setCellValue('I39', ${"Totalprice".$s});
										 $sheet->setCellValue('S39', "合計");
										 $sheet->setCellValue('T39', ${"Totalprice".$s});
								 ...
								 $sheet->setCellValue("T".$num, $arrPros[$i]["price"]);
									 ...
									 if($amari == 0 && $syou == 0){
										 ...
										 $sheet->setCellValue('I37', ${"Totalprice".$s});
										 $sheet->setCellValue('T37', ${"Totalprice".$s});
...
/*
					 ...
					 ->where(['kousinbi >=' => $date_sta, 'kousinbi <=' => $date_fin, 'miseikyuugaku >' => 0, 'delete_flag' => 0])->order(["furigana"=>"ASC"])->toArray();
