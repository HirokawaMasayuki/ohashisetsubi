...
<table width="1300" border="0" align="center" cellpadding="0" cellspacing="0" bordercolor="#E6FFFF">
  <tr style="background-color: #E6FFFF">
    <td>
        ...
        <?= $this->Form->create($customers, ['url' => ['action' => 'seikyuudo']]) ?>
        <br>
        ...
        <table align="center">
          ...
          <tr>
            ...
            <td align="center" bgcolor="#FFFFCC" style="padding: 0.2rem"><?= h(number_format($kurikosi)) ?></td>
            ...
            <td align="center" bgcolor="#FFFFCC" style="padding: 0.2rem"><?= h(number_format($kurikosi + $totalkingakutaxinc)) ?></td>
          </tr>
        </table>
        <br>
        <table align="center">
...
/*
          <tr>
            <td bgcolor="#FFFFCC" style="padding: 0.2rem"><?= $this->Form->control('utizei_flag', array('type'=>'checkbox', 'label'=>false)) ?></td>
            <td align="center" width="380" bgcolor="#FFFFCC" style="font-size: 12pt;padding: 0.2rem"><strong style="font-size: 11pt">内税で出力する場合はチェックを入れてください　　</strong></td>
          </tr>
          */
          ?>
      ...
      <?= $this->Form->control('del_num', array('type'=>'hidden', 'value'=>0, 'label'=>false)) ?>

      <?php for ($i=0;$i<count($arrDelId);$i++): ?>

          <?= $this->Form->control('del_id'.$i, array('type'=>'hidden', 'value'=>$arrDelId[$i], 'label'=>false)) ?>
          <?= $this->Form->control('del_num', array('type'=>'hidden', 'value'=>$i+1, 'label'=>false)) ?>
    ...
    <br>
        ...
        <tr bgcolor="#E6FFFF" >
          ...
          <td style="border: none"><div align="center"><?= $this->Form->submit(__('請求書出力'), array('name' => 'confirm')); ?></div></td>
        </tr>
        </table>
...
<?= $this->Form->control('nyuukingaku', array('type'=>'hidden', 'value'=>$this->request->getData('nyuukingaku'), 'label'=>false)) ?>
<?= $this->Form->control('tyousei', array('type'=>'hidden', 'value'=>$this->request->getData('tyousei'), 'label'=>false)) ?>
<?= $this->Form->control('sousai', array('type'=>'hidden', 'value'=>$this->request->getData('sousai'), 'label'=>false)) ?>
<?= $this->Form->control('kurikosi', array('type'=>'hidden', 'value'=>$kurikosi, 'label'=>false)) ?>
<?= $this->Form->control('totalseikyuu', array('type'=>'hidden', 'value'=>$totalseikyuu, 'label'=>false)) ?>
<?= $this->Form->control('totalkingaku', array('type'=>'hidden', 'value'=>$totalkingaku, 'label'=>false)) ?>
<?= $this->Form->control('totaltax', array('type'=>'hidden', 'value'=>$totaltax, 'label'=>false)) ?>
<?= $this->Form->control('totalkingakutaxinc', array('type'=>'hidden', 'value'=>$totalkingakutaxinc, 'label'=>false)) ?>
